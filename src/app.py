...
from models import get_engine, get_session, Exception_
...
def _load_registry():
    ...
    if os.path.exists(DB_PATH):
        ...
        try:
            rows = session.query(Exception_).all()
            return [r.to_dict() for r in rows]
        finally:
            session.close()
    else:
        print(f"[warning] {DB_PATH} not found — falling back to CSV. "
              f"Run `python db_setup.py` to use the database.")
