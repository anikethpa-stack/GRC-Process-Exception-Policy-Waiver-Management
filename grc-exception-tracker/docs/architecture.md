...
```
                  ┌─────────────────────┐
                  │  exception_registry  │
                  │       .csv           │   (or live DB / ITSM feed in production)
                  └──────────┬───────────┘
                              │
                              ▼
                  ┌─────────────────────┐
...
- No persistence layer (data reloads from CSV on each request) — fine for a 48-hour
  prototype, would move to PostgreSQL for a production deployment, as the doc's Option A
  stack suggests for scaling beyond this scope.
